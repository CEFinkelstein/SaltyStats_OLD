...
database is a dict that uses the keys "fights" and "chars" to point to two
other dicts.

"fights" points to the database of fight records. The fight records sort
fights by tier (X, S, A, B, P), and then by combatants. Combatant keys are
generated by hashing frozensets containing the two combatants. The combatant
keys point to lists containing the winners of matchups. One instance of a
character in that list indicates one win in that matchup. Note that the
combatant keys are generated using Character objects, not strings of character
names.

"chars" points to the database of character records. It sorts characters by
...
class Fight:
    ...
    def __init__(self, p1, p2, tier, search = False):
        ...
        """
        if p1 not in stats["chars"][tier]:
            stats["chars"][tier][p1] = Character(p1, tier)
            writeStats()
        if p2 not in stats["chars"][tier]:
            stats["chars"][tier][p2] = Character(p2, tier)
            writeStats()
        self.player1 = stats["chars"][tier][p1]
        self.player2 = stats["chars"][tier][p2]
        self.tier = tier
        self.winner = None
        self.over = False
        if not search:
    ...
    def searchForRematches(self):
        ...
        if key in stats["fights"][self.tier]:
            fights = stats["fights"][self.tier][key]
    ...
    def recordFightWinner(self):
        ...
        global stats
        key = frozenset([self.player1, self.player2])
        if key not in stats["fights"][self.tier]:
            stats["fights"][self.tier][key] = []
        stats["fights"][self.tier][key].append(self.winner)
...
def loadStats():
    ...
    if os.path.isfile(statfile):
        ...
        if "fights" not in stats:
            stats = {"fights":{"X":{}, "S":{}, "A":{}, "B":{}, "P":{}},
                     "chars":stats}
            cPickle.dump(stats, open(statfile, "wb"))
